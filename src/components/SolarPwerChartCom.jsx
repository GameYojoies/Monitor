...
const SolarPwerChartCom = () => {
  ...
  useEffect(() => {
    if (select === "select1") {
      ...
      setAllDay(
        `/reportData/powerChart?devicePn=${getPin}&type=10&date=${selectdate}`
      )
      setSolarDate(selectdate)
    } else if (select == "select3") {
      setAllDay(
        `/reportData/powerChart?devicePn=${getPin}&type=20&${selectmonth}`
      )
    } else {
      setAllDay(
        `/reportData/powerChart?devicePn=${getPin}&type=30&${selectyear}`
      )
    }
  }, [select, pin])
  ...
  const getAPI = async () => {
    try {
      ...
      if (getData.code === 0) {
        const pvPowerGenerations = response.data.result.map(
          (data) => data.pvPowerGeneration
        )
        const currentPv = response.data.result.map(
          (data) => data.currentLoadPower
        )
    ...
    <div>
      ...
      <div className="flex flex-col gap-2 p-2 items-center justify-center shadow-lg rounded-xl h-[500px] w-[100%] mt-10">
        <div className="w-[90%] mt-8 flex items-center justify-end gap-4">
          <div className="shadow-lg font-semibold text-[#7B94B5] border-2 border-[#DADADA70] flex justify-between items-center h-[45px] w-[350px] rounded-2xl overflow-hidden bg-[#DADADA50] border-1">
            <div
              ...
              className={`w-[25%] h-[100%] flex items-center justify-center border-x-2 border-[#DADADA70] ${
                select === "select2" ? "bg-[#0072D6] text-white" : ""
              }`}>
              <span>Day</span>
            </div>
            <div
              onClick={() => handleSelect("select3")}
              className={`w-[25%] h-[100%] flex items-center justify-center border-l-2 border-[#DADADA70] ${
                select === "select3" ? "bg-[#0072D6] text-white" : ""
              }`}>
              <span>Month</span>
            </div>
            <div
              ...
              className={`w-[25%] h-[100%] flex items-center justify-center border-l-2 border-[#DADADA70] ${
                select === "select4" ? "bg-[#0072D6] text-white" : ""
              }`}>
              <span>Years</span>
            </div>
            <div
              onClick={() => handleSelect("select1")}
              className={`w-[25%] h-[100%] flex items-center justify-center border-r-2 border-l-2 border-[#DADADA70] ${
                select === "select1" ? "bg-[#0072D6] text-white" : ""
              }`}>
              <span>Total</span>
            </div>
          </div>
        ...
        <div className="mt-6 flex items-center h-[400px] w-[95%] relative">
          <span className="inline-block transform -rotate-90 absolute left-[-30px]">
            Power (kWh)
          </span>
          <ReactECharts
            option={option}
